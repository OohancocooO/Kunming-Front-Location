...
for i in range(3):
    ...
    # 提取距离最近的格点数据并存储到字典中
    station_data[f"data_at_station_{i + 1}"] = era5_data.isel(
        latitude=min_idx[0], longitude=min_idx[1]
    )
...
for i in range(3):
    temperature = station_data[f"data_at_station_{i + 1}"].sp  # 把t2m换成sp就是气压图
    t = temperature.sel(
        time=slice("2008-01-12T00:00:00.000000000", "2008-01-13T00:00:00.000000000")
    )
    # 将温度从开尔文转换为摄氏度
    t_celsius = t - 273.15
    len_x = len(t_celsius)
    ax.plot(range(len_x), t_celsius, label=station[i])
    ax.scatter(range(len_x), t_celsius, marker="^")

time_range = [f"{i:02d}:00" for i in range(len_x)]
ax.set_xticks(range(0, len_x, 2), time_range[::2], rotation=20)
# 添加图例
ax.legend()
# 设置标题和轴标签
ax.set_title("2008年1月12日日3个站点温度变化图")
ax.set_xlabel("Time")
